            ...
            for y in min_y..=max_y {
                ...
                for i in 0..4 {
                    if samples[i] {
                        ...
                        let z = alpha * triangle.v[0].z
                            + beta * triangle.v[1].z
                            + gamma * triangle.v[2].z;

                        if self.depth_sample[index][i] > z {
                            self.depth_sample[index][i] = z;

                            let interpolated_color = Rasterizer::interpolate_vec3(
                                alpha,
                                beta,
                                gamma,
                                triangle.color[0],
                            ...
                            let interpolated_normal = Rasterizer::interpolate_vec3(
                                alpha,
                                beta,
                                gamma,
                                triangle.normal[0],
                            ...
                            .normalize();
                            let interpolated_texcoords = Rasterizer::interpolate_vec2(
                                alpha,
                                beta,
                                gamma,
                                triangle.tex_coords[0],
                                triangle.tex_coords[1],
                                triangle.tex_coords[2],
                                1.0,
                            );

                            let payload = FragmentShaderPayload::new(
                                &interpolated_color,
                                &interpolated_normal,
                            ...
                            self.frame_sample[index][i] = match self.fragment_shader {
                                None => Vector3::zeros(),
                                Some(frag_shader) => frag_shader(&payload),
                            };
                        }
                    }
                }
